...
class LineSimulator:
    ...
    def _interpolate_position(
        self, from_idx: int, to_idx: int, progress: float
    ) -> Tuple[float, float]:
    ...
    def _get_target_speed(self, progress: float, at_station: bool) -> float:
        ...
        if progress < ACCEL_END_PROGRESS:
            ...
            # Decelerating phase
            decel_progress = (progress - BRAKE_START_PROGRESS) / (
                1.0 - BRAKE_START_PROGRESS
            )
    ...
    def _update_train(self, train: TrainState, dt: float) -> None:
        ...
        max_speed_change = (
            ACCELERATION_RATE * dt * 3.6
        )  # Convert m/s² to km/h per second
        ...
        if is_braking:
            ...
            train.total_energy_recovered += (
                BASE_ENERGY_RECOVERY_RATE * dt * random.uniform(0.8, 1.2)
            )
            # Temperature increases during braking
            train.regen_temp = min(
                MAX_REGEN_TEMP, train.regen_temp + dt * random.uniform(0.5, 2.0)
            )
        else:
            # Temperature slowly decreases when not braking
            train.regen_temp = max(
                MIN_REGEN_TEMP, train.regen_temp - dt * random.uniform(0.1, 0.5)
            )
    ...
    def get_train_status(self, train: TrainState) -> TrainStatus:
        ...
        lat, lng = self._interpolate_position(
            train.current_station_idx, next_station_idx, train.progress
        )
        ...
        if self.line == MetroLine.LINE_3:
            in_tunnel = is_in_tunnel_section(
                train.current_station_idx, train.progress, self.line, train.direction
            )
        ...
        if train.at_station:
            ...
            eta_seconds = (
                int(remaining_distance / avg_speed_kms) if avg_speed_kms > 0 else 999
            )
        else:
            eta_seconds = 999
